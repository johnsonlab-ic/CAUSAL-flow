...
cellCOLOC=function(GWAS,
  ...
  if(cellMR == TRUE) {
    mr_results = run_cellMR(cellCOLOC_obj, use_coloc_lead_snp = use_coloc_lead_snp, coloc_res = coloc_df, pph4_cutoff = pph4_cutoff, eqtl_FDR_cutoff = 0.05, r2_cutoff = 0.01)
    if(length(mr_results) > 1) {
        write.table(mr_results, paste0(GWAS_name, "_MR_results.txt"))
    }
    if(IVPCA == TRUE & length(mr_results) > 1) {
        message(paste0(Sys.time(), ": Running IVPCA.."))
        mr_res_PCA_99 = run_cellMR_IVPCA(cellCOLOC_obj = cellCOLOC_obj, coloc_res = coloc_df, pph4_cutoff = pph4_cutoff, eqtl_FDR_cutoff = 0.05, percentage_variance = 0.99)
        mr_res_PCA_999 = run_cellMR_IVPCA(cellCOLOC_obj = cellCOLOC_obj, coloc_res = coloc_df, pph4_cutoff = pph4_cutoff, eqtl_FDR_cutoff = 0.05, percentage_variance = 0.999)
        write.table(mr_res_PCA_99, paste0(GWAS_name, "_MR_IVPCA_0.99_results.txt"))
  ...
  # geneName = "BIN1"
    ...
    if(type=="cc"){
      gwas_trait = list(
            ...
            gwas_trait = list(
            beta = as.numeric(gwasData@beta$b[idx]),  
            varbeta = (as.numeric(gwasData@std_error$se[idx]))^2,  # Subset, convert and square
            snp = gwasData@beta$rsid[idx],
            type = cellCOLOC_obj@gwasInfo, # Assuming the type is stored here
            N = cellCOLOC_obj@gwas_N,  # GWAS data doesn't provide this, you might fill it in later
...
path_to_binaries="/rds/general/user/ah3918/projects/single_cell_eqtl/live/MRC/REFERENCE_DATASETS/clumping_ref/EUR/EUR"){
    ...
    for(i in 1:nrow(coloc_res)){
        ...
        # warning("No SNPs left after filtering for eQTL FDR in iteration ", i)
            # MR_results[[i]] = c(celltype,gene,NA,NA,NA,NA)
            next
          }
...
create_gene_plot <- function(gene_df, chromosome, min_pos, max_pos, target_gene) {
  ...
  gene_df$left <- gene_df$left / 1e6
  gene_df$right <- gene_df$right / 1e6
  min_pos <- min_pos / 1e6
  max_pos <- max_pos / 1e6
  ...
  gene_plot <- ggplot() +
    ...
    labs(y = NULL, x = paste0("Position on ", chromosome, " (Mb)")) +
    theme_bw() +
...
# Example for marker_plot_eqtl
marker_plot_eqtl <- marker_plot_eqtl +
  theme(plot.margin = margin(0, 0, 0, 0, "mm"))

# Example for marker_plot_gwas
marker_plot_gwas <- marker_plot_gwas +
  theme(plot.margin = margin(0, 0, 0, 0, "mm"))

# Example for gene_plot
gene_plot <- gene_plot +
  theme(plot.margin = margin(0, 0, 0, 0, "mm"))
